...
namespace ICG
{
	...
	public struct Tiles
	{
		...
		public const int ROAD3WAY = 6;
		public const int ROAD4WAY = 7;
	...
	public class Assets
	{
		...
		private static Texture2D[] _tiles = new Texture2D[8];
		...
		public static Facing RotateDirectionLeft (Facing f)
		{
			for (int x = 0; x < 4; x++) {
				if(_facing[x] == f && x != 3)
					return _facing[x+1];
		...
		public static Facing RotateDirectionRight(Facing f)
		{
			for (int x = 0; x < 4; x++) {
				if(_facing[x] == f && x != 0)
					return _facing[x-1];
